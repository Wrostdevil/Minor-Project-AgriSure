  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden border-b bg-gradient-to-br from-emerald-50 to-amber-50">
        <div className="container py-20 md:py-28 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">Protect your farm with AgriSure</h1>
            <p className="mt-4 text-lg text-muted-foreground max-w-prose">
              Crop, livestock, health and motor insurance built for agriculture. Real‑time weather insights and AI guidance help you decide faster.
            </p>
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <Button size="lg" className="gap-2" asChild>
                <a href="/assistant">Talk to Assistant <ArrowRight className="h-4 w-4" /></a>
              </Button>
              <Button size="lg" variant="outline" asChild>
                <a href="/weather">View Weather Analytics</a>
              </Button>
            </div>
            <div className="mt-10 max-w-md"><WeatherWidget /></div>
          </div>
      ...
      <section className="container py-10 md:py-14">
        <p className="text-center text-sm text-muted-foreground">Backed by partners</p>
        <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-6 items-center opacity-70">
          {['AgriCorp','FieldTrust','GreenLife','FarmTech','Sunrise'].map((p)=> (
            <div key={p} className="h-10 rounded bg-muted/40 flex items-center justify-center text-xs font-semibold">{p}</div>
          ))}
        </div>
      </section>
      ...
      <section className="container py-16 text-center">
        <h3 className="text-2xl md:text-3xl font-bold">Ready to secure your season?</h3>
        <p className="mt-2 text-muted-foreground">Get a quote in minutes or chat with our assistant.</p>
        <div className="mt-6 flex justify-center gap-3">
          <Button size="lg" asChild><a href="/assistant">Get guidance</a></Button>
          <Button size="lg" variant="outline" asChild><a href="/weather">See weather</a></Button>
        </div>
      </section>
    </div>
  );
...
function MiniCard({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle: string }) {
  return (
    <div className="rounded-xl border bg-card p-4">
      <div className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-primary/10 text-primary">{icon}</div>
